...
// Admin's demands are shown LAST; other users' demands are shown first (FIFO order).
app.get("/api/tasks", authMiddleware, async (req, res) => {
  try {
    ...
    // Find the admin user to identify admin's tasks
    const adminUser = await User.findOne({ username: "admin" });
    const adminId = adminUser ? adminUser._id.toString() : null;

    // Separate admin tasks and non-admin tasks, then put admin tasks at the end
    const nonAdminTasks = [];
    const adminTasks = [];
    for (const t of tasks) {
      if (adminId && t.userId && t.userId.toString() === adminId) {
        adminTasks.push(t);
      } else {
        nonAdminTasks.push(t);
      }
    }

    // Non-admin tasks first (FIFO), then admin tasks (FIFO)
    const sortedTasks = [...nonAdminTasks, ...adminTasks];
    res.json(sortedTasks.map(taskToJson));
  } catch (e) {
    console.error(e);
